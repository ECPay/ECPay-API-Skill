...
for i, v in enumerate(aes_data['vectors'], 1):
    ...
    if direction == 'decrypt':
        ...
        status = "PASS ✓" if result == v['expected_url_decoded'] else "FAIL ✗"
        if result != v['expected_url_decoded']: failures += 1
        print(f"  Vector {i}: {status} | {v['name']}")
        if result != v['expected_url_decoded']:
            print(f"    Expected: {v['expected_url_decoded']}")
            print(f"    Got:      {result}")
