...
- [ ] `RqHeader` 有 `Revision: "1.0.0"`（與站內付2.0 不同，物流 v2 需要 Revision）
...
// $result 包含：RtnCode, LogisticsID, LogisticsStatus 等
...
$responseBody = json_encode([
    ...
    'TransCode'  => 1,
    'TransMsg'   => '',
    'Data'       => $aesService->encrypt(['RtnCode' => 1, 'RtnMsg' => '成功']),
]);
...
app.post('/ecpay/logistics-notify', (req, res) => {
  // ⚠️ ECPay 以 JSON body 傳送 callback（不是 Form POST）；讀取 Data 欄位後 AES 解密
  const encryptedData = (req.body && req.body.Data) || '';
  let result;
  try {
