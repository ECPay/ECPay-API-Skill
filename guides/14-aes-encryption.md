...
Python: line 246-292 | Node.js: line 294-341 | TypeScript: line 343-396
Java: line 398-475 | C#: line 477-538 | Go: line 540-644
C: line 646-820 | C++: line 822-975 | Rust: line 977-1041
Swift: line 1043-1118 | Kotlin: line 1120-1167 | Ruby: line 1169-1217
Test vectors: line 1219-1381 | 常見錯誤: line 1383-1414
...
> 電子票證例外：需三層驗證（TransCode → CheckMacValue → RtnCode），見 [guides/09](./09-ecticket.md)。
...
| **`~` 處理** | **`%7E`（大寫 hex）**：PHP urlencode 自動編碼；Python/Node.js 需手動加 `.replace('~', '%7E')` | **`%7e`（小寫 hex）**：urlencode 後 strtolower 轉小寫 |
| `!` `*` `'` `(` `)` | **`%21 %2A %27 %28 %29`**（AES 保留編碼，不還原）| `!` `*` `'` `(` `)`（.NET 替換還原為原始字元）|
...
> **Ruby CGI.escape 注意**：`CGI.escape` 安全字元集為 `a-zA-Z0-9_.-`（**`~` 會被 CGI.escape 編碼為 `%7E`**，空格→`+`）。
