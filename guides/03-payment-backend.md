...
| 功能 | 端點路徑 | 官方 SDK 範例 |
|------|---------|-------------|
| 取得綁卡 Token | `/Merchant/GetTokenbyBindingCard` | `GetTokenbyBindingCard.php` |
| 查詢會員綁卡 | `/Merchant/GetMemberBindCard` | `GetMemberBindCard.php` |
| 刪除綁卡 | `/Merchant/DeleteMemberBindCard` | `DeleteMemberBindCard.php` |
| 綁卡代扣 | `/Merchant/CreatePaymentWithCardID` | `CreatePaymentWithCardID.php` |
...
| ItemName | string | ✅ | 商品名稱（多件以 `#` 分隔，上限 400 字元） |
| CardNo | string | ✅ | 信用卡卡號 |
...
| 退款 | `/1.0.0/Credit/DoAction` | R | 退款（**隔日後**，已請款交易） |
| 取消授權 | `/1.0.0/Credit/DoAction` | E | 取消尚未請款的授權 |
| 放棄 | `/1.0.0/Credit/DoAction` | N | **當天**取消/放棄請款 |
| 查詢 | `/1.0.0/CreditDetail/QueryTrade` | — | 查詢信用卡交易明細 |

> ⚠️ **退款時機說明**：同日取消使用 `Action=N`（放棄）；隔日後退款使用 `Action=R`（退款）。**ATM/超商代碼/條碼付款不支援 DoAction**，非信用卡無 API 退款機制（見下節）。
