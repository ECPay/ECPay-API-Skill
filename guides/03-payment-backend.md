...
> 📌 解密後的 Data 物件結構同 [guides/02](./02-payment-ecpg.md) §CreatePayment 回應。
...
> ⚠️ **參數結構注意**：上方表格列出的是業務參數（平面呈現），實際 AES-JSON 請求中這些參數會包在巢狀物件內（`Data.OrderInfo`、`Data.CardInfo`、`Data.ConsumerInfo`），經 AES 加密後放入三層 JSON 的 `Data` 欄位傳送。參照上方步驟 2 的程式碼範例了解完整的 參數 → AES 加密 → POST 流程。
