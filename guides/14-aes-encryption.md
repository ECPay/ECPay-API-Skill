...
Python: line 233-279 | Node.js: line 281-328 | TypeScript: line 330-383
Java: line 385-462 | C#: line 464-525 | Go: line 527-631
C: line 633-791 | C++: line 793-946 | Rust: line 948-1012
Swift: line 1014-1089 | Kotlin: line 1091-1138 | Ruby: line 1140-1188
Test vectors: line 1190-1334 | 常見錯誤: line 1336-1367
...
> - **`aesUrlEncode` vs `ecpayUrlEncode`**：ECPG 各服務使用 `aesUrlEncode`（只做 `urlencode`，不做 lowercase 和 .NET 字元替換）；AIO 金流使用 `ecpayUrlEncode`。**兩者絕對不可混用**，否則 `TransCode ≠ 1`。
...
| 全方位物流 | 1.0.0 | — |
| 跨境物流 | 1.0.0 | — |
| 電子票證 | 1.0.0 | 外層另附 CheckMacValue（SHA256，公式見 [guides/09](./09-ecticket.md)） |
...
| `~` 處理 | PHP：`~→%7E`（`urlencode` 不編碼 `~`，需手動替換）<br>Python：額外加 `'→%27`（`quote_plus` 已編碼其他字元）<br>Node.js：額外加 `'→%27`（`encodeURIComponent` 不編碼 `~`，需替換） — **見下方各語言實作** | `~→%7e`（urlencode 後 strtolower 轉小寫） |
...
以下實作從 `AesService.php` 精確翻譯，涵蓋 Python、Node.js、TypeScript、Java、C#、Go、C、C++、Rust、Swift、Kotlin、Ruby。
...
public static class EcpayAes
{
    public static string Encrypt(string jsonStr, string hashKey, string hashIv)
    {
        ...
        string urlEncoded = WebUtility.UrlEncode(jsonStr)
            ?.Replace("%20", "+")     // 保險：確保空格為 +（WebUtility 空格已為 +，與 PHP 相同）
            .Replace("~", "%7E").Replace("!", "%21").Replace("*", "%2A")
            .Replace("'", "%27").Replace("(", "%28").Replace(")", "%29") ?? "";
        // 3. AES-128-CBC + PKCS7
...
char* ecpay_aes_encrypt(const char* json_str, const char* hash_key, const char* hash_iv) {
    ...
    /* curl_easy_escape 不編碼 ~ (RFC 3986 unreserved)，但 PHP urlencode 會編碼為 %7E；
       !*'() 已被 curl_easy_escape 按 RFC 3986 sub-delimiters 編碼，以下 str_replace 為冪等保險 */
...
> **Ruby CGI.escape 注意**：`CGI.escape` 安全字元集為 `a-zA-Z0-9_.-~`，**不編碼 `~`**（空格→`+`）。
> `~` 需手動 `.gsub('~', '%7E')` 補齊 PHP `urlencode` 行為；`!*'()` 已由 `CGI.escape` 編碼，後續 `.gsub` 為冪等安全措施。
