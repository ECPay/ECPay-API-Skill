...
| **直播收款** | ReturnURL | `1\|OK`（純文字）⚠️ 驗證方式同電子票證（AES 解密 Data + ECTicket 式 CMV SHA256），但回應為純文字而非 AES-JSON | text/plain | 每 5-15 分鐘重送，每日最多 4 次 |
...
| **直播收款** | ReturnURL | 付款通知 | AES 解密 Data + **ECTicket 式** CheckMacValue (SHA256)（格式與電子票證相同） | `1\|OK`（⚠️ 純文字，與電子票證回應不同） | 每 5-15 分鐘重送，每日最多 4 次 |
...
$outerJson  = $aesService->decrypt($resultData);         // 第一層解密：取得外層 JSON（含 TransCode, Data）
$innerJson  = $aesService->decrypt($outerJson['Data']);  // 第二層解密：取得 RtnCode 等業務資料
// 顯示付款結果頁面給消費者
```
...
def ecpg_order_result_url():
    ...
    outer = aes_decrypt(result_data)      # 第一層解密：取得外層 JSON（含 TransCode, Data）
    inner = aes_decrypt(outer['Data'])    # 第二層解密：取得 RtnCode 等業務資料
...
func ecpgOrderResultURL(w http.ResponseWriter, r *http.Request) {
    ...
    outer, _ := aesDecrypt(resultData)              // 第一層解密：取得外層 JSON（含 TransCode, Data）
    inner, _ := aesDecrypt(outer["Data"].(string))  // 第二層解密：取得 RtnCode 等業務資料
