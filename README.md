...
| Claude Code | Claude Pro / Max / Team / Enterprise 訂閱 | 工程師 | [安裝文件](https://docs.anthropic.com/en/docs/claude-code/overview) |
...
| OpenClaw | Node ≥ 22；OpenClaw 安裝完成（免費開源的 AI agent 平台，支援 WhatsApp / Telegram / LINE / Slack / Discord 等通訊頻道；[docs.openclaw.ai](https://docs.openclaw.ai)） | 工程師 | [SETUP.md §OpenClaw](./SETUP.md#openclaw-安裝) |
| ChatGPT GPTs（custom GPT）| 可建立 GPTs 的 ChatGPT 方案：Plus / Pro / Team / Enterprise / Edu（依工作區權限）| 需長期使用或分享 | [GPT Builder](https://chatgpt.com/gpts/editor) |

## 這是什麼？

ECPay Skill 是一個 **AI 知識套件**——安裝到 AI 程式開發助手（Claude Code、VS Code Copilot Chat、GitHub Copilot CLI、Cursor、Windsurf 等），或透過 ChatGPT **GPT Builder**、**Google AI Studio** 等平台上傳後，AI 就能根據你的需求，直接生成綠界 API 串接程式碼、診斷錯誤、引導完整串接流程。

不需要自己翻文件，用自然語言描述需求即可。

### AI 程式開發助手是什麼？

AI 程式開發助手是安裝在開發者電腦上（終端機或程式碼編輯器內）的 AI 工具，能讀取專案程式碼、用自然語言對話、直接生成或修改程式碼。它不是瀏覽器裡的 ChatGPT——而是嵌入開發工作流程的專業工具。上方「前置需求」表格列出的 Claude Code、VS Code Copilot Chat、GitHub Copilot CLI、Cursor、Windsurf 等都屬於這類工具。
...
> | Windsurf | `git clone ...ECPay-API-Skill.git .windsurf/skills/ecpay` | 同上 |
> | OpenClaw | `git clone ...ECPay-API-Skill.git ~/.openclaw/skills/ecpay` | 同上 |
...
> 驗證：開啟 Claude Code，輸入 `/ecpay` 或問「What skills are available?」，確認 ECPay Skill 出現在清單中。
...
# 建立 Custom Instructions（將 ECPay Skill 知識載入 Copilot）
...
詳細步驟（含設定驗證）見 [`SETUP.md §OpenClaw`](./SETUP.md#openclaw-安裝)。
...
# 之後如需升級
git fetch --tags && git checkout v1.5
```
...
├── SETUP.md                    # 統一安裝指南（Codex CLI / Gemini CLI / OpenClaw / ChatGPT GPTs）
├── README.md                   # 本文件
