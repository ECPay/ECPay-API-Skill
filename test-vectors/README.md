...
- URL Encode 差異: [guides/14-aes-encryption.md](../guides/14-aes-encryption.md) §AES vs CMV URL Encode 對比表（對應 [url-encode-comparison.json](url-encode-comparison.json)）
...
| 8 | **AllowanceByCollegiate MD5** | B2C 發票唯一帶 CMV 的 API，使用 MD5 + 發票帳號（非 AIO 帳號）|
