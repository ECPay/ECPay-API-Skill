...
| ReturnURL Callback | JSON POST (`application/json`) + AES 解密 Data + ECTicket 式 CheckMacValue SHA256（`key+Data明文+iv`）— 回應 `1\|OK` |
...
- **付款通知**：消費者付款後，綠界以 **JSON POST** 傳送到 ReturnURL（格式同 ECTicket：AES 解密 Data + ECTicket-式 CMV，**非** AIO Form POST 格式；需回應 `1|OK`）
