...
23. **Callbacks must be idempotent** — ECPay may resend callbacks up to 4 times on network failures. Use `MerchantTradeNo` as unique key with upsert (not insert) to prevent duplicate processing.
