...
> - **OrderResultURL**（瀏覽器 Form POST, Content-Type: application/x-www-form-urlencoded）：讀取 `ResultData` 表單欄位（不是 JSON body）→ AES 解密 → 顯示結果頁面
...
| `PaymentUIType` | Int | ✅ 必填 | `0`=定期定額 `2`=付款選擇清單頁（一般使用）`5`=Apple Pay 延遲付款 |
...
| `CardInfo.Redeem` | String(1) | — 選填 | `'0'`=不使用紅利（預設）`'1'`=使用紅利（**字串型別**，官方規格 String(1)）|
...
> | **OrderResultURL** | 消費者瀏覽器（表單跳轉） | `application/x-www-form-urlencoded` | `$_POST['ResultData']` → AES 解密 | 無需（顯示結果頁面） |
...
```php
// ← 讀表單欄位 ResultData，不是 JSON body
$encryptedData = $_POST['ResultData'] ?? '';
$aesService = $factory->create(\Ecpay\Sdk\Services\AesService::class);
$data = $aesService->decrypt($encryptedData);
// 顯示結果頁面（不需回應 1|OK）
...
> | OrderResultURL 解析失敗 | 用 `request.json()` 讀 Form POST | 改用 `$_POST['ResultData']`（表單欄位），再 AES 解密 |
...
app.post('/ecpay/result', express.urlencoded({ extended: true }), (req, res) => {
    // ⚠️ Form POST 讀 req.body.ResultData（表單欄位），不是 JSON body
    const resultData: string = req.body.ResultData;
    const data = aesDecrypt(resultData);
...
>
> **OrderResultURL**（瀏覽器跳轉，消費者發送）：收到 `Content-Type: application/x-www-form-urlencoded` 的 POST，表單欄位 `ResultData` 是 AES 加密字串；解密後 `RtnCode === 1`。**不需回應 `1|OK`，直接顯示結果頁面給消費者。**
>
...
> - OrderResultURL → `request.form['ResultData']` → AES 解密 → 回應 HTML 頁面
> 兩個路由的讀取方式**絕對不可互換**。
...
@app.route('/ecpay/result', methods=['POST'])
def order_result():
    # ⚠️ 讀取方式：request.form['ResultData']（不是 get_json）
    cipher_text = request.form.get('ResultData', '')
    data = aes_decrypt(cipher_text)
...
app.post('/ecpay/callback', (req, res) => {
  const body = req.body;
  if (!body?.Data) return res.status(400).send('Bad Request');
  const data = aesDecrypt(body.Data);
...
app.post('/ecpay/result', (req, res) => {
  const data = aesDecrypt(req.body.ResultData || '');
  if (safeEqual(data.RtnCode, 1))
...
export async function POST(req: NextRequest) {
  ...
  const cipherText = formData.get('ResultData') as string;
  const data = aesDecrypt(cipherText);
  // 重導到結果頁（帶查詢參數，讓前端渲染）
...
> 🔍 **此步驟失敗？** TransCode ≠ 1 → 排查見 [§15 TransCode 診斷](15-troubleshooting.md#15-站內付20-transcode-1-診斷)；HTTP 404 → 確認 Domain 為 `ecpg-stage.ecpay.com.tw`（非 `ecpayment`）。
...
> 🔍 **此步驟失敗？** ①確認 `TransCode == 1`；② 解密 `Data` 後若 `ThreeDURL` 非空，**必須導向該 URL** 完成 3D 驗證，否則交易逾時失敗；③完整排查見 [§16 ThreeDURL 處理](15-troubleshooting.md#16-站內付20-threedurl-遺漏導致交易逾時)。
...
> 🔍 **此步驟失敗？** OrderResultURL 常見誤用 `json_decode` 取代 `$_POST['ResultData']`；ReturnURL 忘記回應 `1|OK`；AES 解密失敗請確認 Key/IV 取自 `HashKey`/`HashIV`（非 AIO 的 HashKey）。詳細排查見 [§17 Callback 格式混淆](15-troubleshooting.md#17-站內付20-callback-格式混淆)。
...
詳細 App SDK 規格見：`references/Payment/站內付2.0API技術文件App.md`（41 個 URL）
...
window.ECPayPayment.createPayment({
  token: currentToken,
  onError: (err) => {
