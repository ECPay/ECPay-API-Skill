...
Python: line 135-188 | Node.js: line 190-250 | TypeScript: line 252-318
Java: line 320-383 | C#: line 385-463 | Go: line 465-540
C: line 542-712 | C++: line 713-835 | Rust: line 836-916
Swift: line 917-1001 | Kotlin: line 1002-1062 | Ruby: line 1063-1113
Test vectors: line 1114-1224
...
| 電子票證 | SHA256 | 送出請求與回應都需附加（**公式與 AIO 完全不同**：`SHA256(toLowerCase(urlencode(HashKey + JSON + HashIV)))`，無排序、無 .NET 替換，**輸出同 AIO 為大寫**（`toLowerCase` 僅作用於 URL 編碼的輸入字串，不影響 SHA256 的輸出），見 [guides/09 §CheckMacValue 計算](./09-ecticket.md)） |
...
> 從 `scripts/SDK_PHP/src/Services/CheckMacValueService.php` 精確對應

```
1. filter()                    — 移除參數中既有的 CheckMacValue
2. sort()                      — Key 不區分大小寫字典序排序（strcasecmp / localeCompare）
3. toEncodeSourceString()      — "HashKey={key}&{k1=v1&k2=v2&...}&HashIV={iv}"
4. UrlService::ecpayUrlEncode()— urlencode → 轉小寫 → .NET 特殊字元替換（見下方 §ECPay 專用 URL Encode）
...
> 從 `scripts/SDK_PHP/src/Services/UrlService.php` 精確對應
