...
>
> ⚠️ **欄位結構依 IssueType 不同**：下方 PHP 範例為提貨券（IssueType=1 or 2，含 ValidDate）的簡化示範；IssueType=4（純序號）的欄位結構請參考上方 Python 範例（使用 `TicketInfo` 陣列和 `TicketAmount`）。各 IssueType 的完整必填欄位請 `web_fetch references/Ecticket/` 對應文件確認。
