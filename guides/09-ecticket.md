...
- [ ] 了解三層驗證流程：TransCode → **CheckMacValue（SHA256）** → RtnCode
- [ ] AES-128-CBC 已實作（見 [guides/14](./14-aes-encryption.md)）；CheckMacValue 計算見 [guides/13](./13-checkmacvalue.md)
...
def post_ecticket(endpoint: str, data: dict) -> dict:
    ...
    # 回應 CMV 以 E-Ticket 公式計算 Data 明文（見 §CheckMacValue 計算 情境 A）
    received_cmv   = res.pop('CheckMacValue', '')
    resp_data      = aes_decrypt(res['Data'])
    resp_data_json = json.dumps(resp_data, ensure_ascii=False, separators=(',', ':'))
    import hmac as _hmac
    if not _hmac.compare_digest(calc_ecticket_cmv(resp_data_json), received_cmv.upper()):
        raise ValueError('CheckMacValue 驗證失敗（回應偽造或 HashKey 錯誤）')

    # 第三重：確認業務層 RtnCode
    return resp_data

# 發行票券（IssueType=4 純序號範例）
try:
    data = post_ecticket('/api/Ticket/Issue', {
        'MerchantID':         MERCHANT_ID,
        'MerchantTradeNo':    'EC' + str(int(time.time())),  # 提貨券必填，純序號必填
    ...
    if data.get('RtnCode') == 1:  # 整數比較（AES-JSON 解密後為 Int）
        print(f"✅ 票券發行成功！TicketNo={data.get('TicketNo')}")
    else:
...
> ✅ **成功時的預期輸出**：
> ```
> ✅ 票券發行成功！TicketNo=EC202603130001
> ```
...
> | RtnCode ≠ 1 | 業務參數錯誤，查 [guides/21](./21-error-codes-reference.md) |
...
```php
// 驗證收到的通知 TransCode → 驗證 CheckMacValue（ECTicket 公式）→ 解密 Data → 處理核銷 → 回應
// ⚠️ HashKey/HashIV 必須與 Factory 設定一致（ECTicket 專用帳號）
$hashKey  = '7b53896b742849d3';  // 請替換為您的 HashKey
$hashIv   = '37a0ad3c6ffa428b';  // 請替換為您的 HashIV
...
**API 端點**（14 個功能）— 端點來源：官方 API 技術文件
...
// 公式：SHA256( toLowerCase( URLEncode( HashKey + DataJson + HashIV ) ) )
...
try {
    $raw      = $curlService->run($outerJson, 'https://ecticket-stage.ecpay.com.tw/api/Ticket/Issue');
    $response = json_decode($raw, true);
    // 三層驗證：TransCode → CheckMacValue → 解密 Data → RtnCode（詳見 §三重驗證）
} catch (\Exception $e) {
...
try {
    $raw      = $curlService->run($outerJson, 'https://ecticket-stage.ecpay.com.tw/api/Ticket/Redeem');
    $response = json_decode($raw, true);
} catch (\Exception $e) {
...
> 倉庫內 `scripts/SDK_PHP/example/Ecticket/README.md` 提供的是高階整合提醒；實作驗證以此處公式與 `test-vectors/checkmacvalue.json` 的 **E-Ticket CMV 公式測試** 為準。
...
```
CheckMacValue = SHA256( toLowerCase( URLEncode( HashKey + Data明文JSON + HashIV ) ) )
```
...
$curlService = $factory->create('JsonCurlService');
$raw         = $curlService->run(json_encode($input), 'https://ecticket-stage.ecpay.com.tw/api/Ticket/Issue');
$response    = json_decode($raw, true);
```
