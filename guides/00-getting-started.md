...
| **ECPG** | EC Payment Gateway 的簡稱，即綠界的**線上金流服務**，涵蓋站內付 2.0（Web/App）、綁定信用卡、幕後授權等服務。注意：ECPG ≠ 站內付 2.0。POS 刷卡機屬於線下金流，不在 ECPG 範圍內 |
...
| 服務 | 說明 | 適用場景 |
|------|------|---------|
| 金流 | 信用卡、ATM、超商代碼、條碼、WebATM、TWQR、BNPL、微信、Apple Pay、銀聯 | 線上收款（ECPG）；線下收款（POS 刷卡機） |
...
| **AES-JSON** | 像加密信件——把整段資料用密鑰鎖起來（AES 加密），放進 JSON 信封再寄出 | ECPG / 發票 / v2 物流 / 幕後授權 + 幕後取號 / 跨境 | ★★★ |
...
**5 步驟快速路徑**（完整說明 + 每步驟失敗排查見 [guides/02 §首次串接快速路徑](./02-payment-ecpg.md)）：
...
| 非 PHP 語言完整範例 (Go/Java/C#/TS/Kotlin/Ruby) | [guides/24-multi-language-integration.md](./24-multi-language-integration.md)（⚠️ 999 行，使用 AI Section Index 跳轉） |
| 完整決策樹 | SKILL.md 步驟 2 |
...
> | 電子發票 B2C/B2B | 2000132 | ejCk326UnaZWKisg | q9jcZX8Ib9LM8wYk | AES |
> | 物流 B2C/宅配 | 2000132 | 5294y06JbISpM5x9 | v77hoKGq4kWxNNIS | MD5 |
...
> | 站內付2.0（AES-JSON） | ReturnURL | `1\|OK` |
> | 幕後授權（AES-JSON） | ReturnURL | `1\|OK` |
> | 幕後取號（AES-JSON） | PaymentInfoURL | 不需回應特定格式（非重試型） |
> | 全方位/跨境物流（AES-JSON v2） | ServerReplyURL | AES 加密 JSON（見 guides/07） |
> | 國內物流（CMV-MD5） | ServerReplyURL | `1\|OK` |
> | 電子票證（AES-JSON + CMV） | ReturnURL | AES 加密 JSON + ECTicket 式 CMV |
> | 直播收款（AES-JSON） | ReturnURL | AES 加密 JSON 解密驗簽，回應 `1\|OK` |
> | AllowanceByCollegiate（發票） | NotifyURL | MD5 CMV（Form POST）— 發票 API 中唯一附帶 CMV 的 Callback |
>
...
| **ECPG** | EC Payment Gateway | 綠界的線上金流服務簡稱，涵蓋站內付 2.0（Web/App）、綁定信用卡、幕後授權等服務。ECPG ≠ 站內付 2.0。POS 刷卡機屬於線下金流，不在 ECPG 範圍內 |
