...
- [ ] ⚠️ **RqHeader 與 B2C 不同**：B2B 多了 `RqID`（UUID 格式，每次請求唯一），且 `Revision` 為 `"1.0.0"`（B2C 是 `"3.0.0"`）
...
try {
    ...
    // ✅ 成功：$response['TransCode'] === 1 且解密後 RtnCode === 1
...
> PHP 可用 `\Ramsey\Uuid\Uuid::uuid4()->toString()` 或 PHP 原生 `\Ramsey\Uuid\Uuid::uuid4()->toString()` 搭配格式化。
