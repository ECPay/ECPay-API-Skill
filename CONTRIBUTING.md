...
2. 修改時遵守以下原則：
   ...
   - **CI 驗證範圍**：所有 `guides/` 下的修改都會觸發完整 CI pipeline（validate.yml），其中 AI Section Index 行號驗證步驟（validate-ai-index.sh）只驗證 guides/13、14、24；其餘 25 份 guide 的修改仍會觸發 CI，但不會執行 AI Section Index 行號驗證，修改後可手動執行 `bash scripts/validate-ai-index.sh` 確認（若有修改章節結構）
...
guides/13-checkmacvalue.md、guides/14-aes-encryption.md、guides/24-multi-language-integration.md 頂部有 AI Section Index HTML 注釋，記錄各語言/章節的精確行號。每次編輯這些檔案後必須更新。

### 何時需要更新

- 在 guides/13、14、24 中的任何位置**新增或刪除行**後
...
| 新增語言支援 | guides/13 + guides/14 + guides/24 + SKILL.md（語言數量）+ guides/00 |
