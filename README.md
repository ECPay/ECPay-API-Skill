...
| 平台 | 需求 | 安裝指引 |
|------|------|---------|
| Claude Code | Claude Pro / Max / Team / Enterprise 訂閱 | [安裝文件](https://docs.anthropic.com/en/docs/claude-code/overview) |
| GitHub Copilot CLI | GitHub Copilot Pro 以上訂閱（Free 方案不含 CLI） | [安裝文件](https://docs.github.com/en/copilot/how-tos/copilot-cli/set-up-copilot-cli/install-copilot-cli) |
| Cursor | Cursor Pro / Teams 訂閱 | [下載頁面](https://cursor.com/download) |
| Windsurf | Windsurf Pro / Teams 訂閱 | [下載頁面](https://windsurf.com/download) |
| OpenAI Codex CLI | OpenAI 帳號；`npm install -g @openai/codex` | [CODEX_SETUP.md](./CODEX_SETUP.md) |
| Google Gemini CLI | Google 帳號；`npm install -g @google/gemini-cli` | [GEMINI_SETUP.md](./GEMINI_SETUP.md) |
| OpenClaw | Node ≥ 22；OpenClaw 安裝完成（免費開源的 AI agent 平台，支援 WhatsApp / Telegram / LINE / Slack / Discord 等通訊頻道；[docs.openclaw.ai](https://docs.openclaw.ai)） | [OPENCLAW_SETUP.md](./OPENCLAW_SETUP.md) |
| ChatGPT GPTs（custom GPT）| 可建立 GPTs 的 ChatGPT 方案：Plus / Pro / Team / Enterprise / Edu（依工作區權限）| [GPT Builder](https://chatgpt.com/gpts/editor) |

## 這是什麼？

ECPay Skill 是一個 **AI 知識套件**——安裝到 AI 程式開發助手（Claude Code、GitHub Copilot CLI、Cursor、Windsurf 等），或透過 ChatGPT **GPT Builder** 的 Instructions + Knowledge files 機制建立專用 GPT 後，AI 就能根據你的需求，直接生成綠界 API 串接程式碼、診斷錯誤、引導完整串接流程。

不需要自己翻文件，用自然語言描述需求即可。

### AI 程式開發助手是什麼？

AI 程式開發助手是安裝在開發者電腦上（終端機或程式碼編輯器內）的 AI 工具，能讀取專案程式碼、用自然語言對話、直接生成或修改程式碼。它不是瀏覽器裡的 ChatGPT——而是嵌入開發工作流程的專業工具。上方「前置需求」表格列出的 Claude Code、GitHub Copilot CLI、Cursor、Windsurf 等都屬於這類工具。

### AI Skill 是什麼？

打個比方：AI 程式開發助手就像一個**聰明但對你的業務一無所知的新進工程師**。AI Skill 就是交給他的「**工作手冊**」——安裝 ECPay Skill 後，這個 AI 就變成了熟悉綠界全系列 API 的串接專家。
...
> 💡 **自動載入**：本 repo 的 `.github/copilot-instructions.md` 會被 GitHub Copilot CLI 在此 repo 中工作時**自動讀取**——若你已 clone 本 repo 並在其中開發，Copilot 無需任何額外設定即可感知 ECPay Skill 知識。
>
> 若要在**其他專案**中使用 ECPay Skill，需手動將 ECPay Skill 的指令內容引入你的目標專案：
...
> ⚠️ GitHub Copilot CLI 只讀取**專案根目錄**的 `.github/copilot-instructions.md`，不會自動讀取子目錄的 instructions 檔案。因此，clone 到 `.github/skills/ecpay/` 後，需在目標專案的根目錄 `.github/copilot-instructions.md` 中明確引用，Copilot 才能載入 ECPay Skill 知識。

**Cursor**
```bash
# 專案層級（提交至 repo，團隊共用）
git clone https://github.com/ECPay/ECPay-API-Skill.git .cursor/skills/ecpay
...
```bash
# 專案層級（提交至 repo，團隊共用）
git clone https://github.com/ECPay/ECPay-API-Skill.git .windsurf/skills/ecpay

# 或個人全域安裝（所有專案共享，注意：Windsurf 全域路徑為 ~/.codeium/windsurf/skills/）
git clone https://github.com/ECPay/ECPay-API-Skill.git ~/.codeium/windsurf/skills/ecpay
```

安裝後，在 Windsurf 中詢問「綠界 AIO 金流的測試 MerchantID 是多少？」，若回應為 `3002607` 表示 Skill 已載入。

**OpenClaw**
```bash
# 安裝至個人共用 skills（所有 agent 共享，推薦）
git clone https://github.com/ECPay/ECPay-API-Skill.git ~/.openclaw/skills/ecpay

# 或 workspace 層級（僅當前專案）
git clone https://github.com/ECPay/ECPay-API-Skill.git skills/ecpay
```

詳細步驟（含設定驗證）見 [`OPENCLAW_SETUP.md`](./OPENCLAW_SETUP.md)。

**ChatGPT GPTs（custom GPT）**

1. 開啟 [GPT Builder](https://chatgpt.com/gpts/editor)，建立新的 GPT
2. 在 Configure → **Instructions** 欄位，貼入 `SKILL_OPENAI.md` 的完整內容
3. 在 Configure → **Knowledge**，上傳 `guides/`、`references/` 等關鍵檔案（最多 20 個）
4. 詳細步驟（含建議上傳的檔案清單）見 [`OPENAI_SETUP.md`](./OPENAI_SETUP.md)
...
git checkout v1.0   # 固定至 v1.0

# 之後如需升級
git fetch --tags && git checkout v1.1
```

> 查看所有可用版本：`git tag -l`。開發環境使用 `git pull` 取得最新版即可。
...
- **18 個**跨語言加密測試向量（CheckMacValue × 8、AES × 6、URL Encode × 4），CI 自動驗證（`validate.yml`）
...
- **`scripts/validate-agents-parity.sh`**：驗證 AGENTS.md（Codex CLI 入口）與 GEMINI.md（Gemini CLI 入口）的決策樹、關鍵規則、測試帳號三大區段是否一致。修改任一文件後執行：
  ...
  ```
- **`test-vectors/verify.py`**：一次驗證所有 18 個加密測試向量（CheckMacValue / AES / URL Encode）。CI（validate.yml）自動執行；本機執行需先安裝依賴後執行：
  ```bash
...
| 4 | `test-vectors/verify.py` | 18 個加密測試向量（CheckMacValue / AES / URL Encode）的計算結果是否與預期完全吻合 |
...
> **注意**：直播收款的 API **請求**使用 AES-JSON（無 CMV）；其 ReturnURL **callback 驗證**需使用 ECTicket 式 CheckMacValue（公式同電子票證），詳見 [guides/18](./guides/18-livestream-payment.md)。
...
| 02 | guides/02-payment-ecpg.md | 站內付 2.0——ECPG 服務之一（24 個 PHP 範例） |
...
```
ecpay-skill/
├── SKILL.md                    # AI 進入點（Claude Code / Cursor / Windsurf / OpenClaw）：決策樹 + 導航
...
├── OPENAI_SETUP.md             # ChatGPT GPTs 建置指南
├── CODEX_SETUP.md              # OpenAI Codex CLI 安裝指引
├── GEMINI_SETUP.md             # Google Gemini CLI 安裝指引
├── OPENCLAW_SETUP.md           # OpenClaw 安裝指引
├── README.md                   # 本文件
├── CONTRIBUTING.md             # 貢獻指南
├── SECURITY.md                 # 安全政策（漏洞通報、憑證安全須知）
├── LICENSE                     # All Rights Reserved
├── .github/                    # GitHub 社群模板（Issue/PR 模板、CI workflow）
│   └── copilot-instructions.md # GitHub Copilot CLI AI 自動載入入口（在此 repo 工作時自動生效）
├── test-vectors/               # 跨語言加密驗證用測試向量（CMV + AES + URL Encode，共 18 個）
│   └── verify.py               # 一鍵驗證所有向量（pip install pycryptodome 後執行）
├── commands/                   # Claude Code 快速指令（6 個 /ecpay-* 指令）
├── guides/                     # 25 份深度整合指南（ChatGPT GPTs 依 OPENAI_SETUP.md 選擇上傳子集）
...
| 平台 | AI 入口點 | 說明 |
|------|---------|------|
| Claude Code、Cursor、Windsurf、OpenClaw | `SKILL.md` | 主要決策樹：安裝後 AI 自動讀取 |
...
| ChatGPT GPTs | `SKILL_OPENAI.md` | 貼入 GPT Builder Instructions 欄位 |
...
| OpenClaw | 依後端 LLM 能力而定 | 支援 web_fetch 的後端（如 Claude）直接讀取；其他後端同 ChatGPT GPTs 以 Web Search 替代 |
...
| 服務 | MerchantID | 用途 |
|------|-----------|------|
| **金流（AIO / 站內付 2.0 / 幕後授權）** | `3002607` | 信用卡、ATM、超商代碼等全付款方式測試 |
| **電子發票** | `2000132` | B2C / B2B 發票開立測試 |
| **物流 B2C / 全方位 / 宅配** | `2000132` | 超商取貨 / 宅配 / 全方位物流測試 |
| **物流 C2C** | `2000933` | 超商交貨便測試 |
| **電子票證** | `3085676` | 電子票證（AES-JSON + CMV 協議）測試 |

> 💡 **快速測試提示**：建立訂單時設定 `SimulatePaid=1`，無需真實信用卡即可模擬付款成功。

HashKey / HashIV 及測試信用卡號等完整測試帳號資訊，請見 [guides/00-getting-started.md §測試帳號](./guides/00-getting-started.md) 或在 AI 助手中直接詢問「ECPay 測試帳號」。
