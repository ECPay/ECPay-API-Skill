...
metadata:
  {
    ...
    "platforms": ["claude-code", "github-copilot", "cursor", "windsurf", "openai-gpts", "openclaw", "codex-cli", "gemini-cli"],
    "openclaw": {
      "emoji": "💳",
      "homepage": "https://github.com/ECPay/ECPay-API-Skill"
    }
...
>
> 📌 **ChatGPT GPTs 使用者**：請改用 [`SKILL_OPENAI.md`](./SKILL_OPENAI.md) 作為 GPT Builder 的 Instructions，
> 並依 [`OPENAI_SETUP.md`](./OPENAI_SETUP.md) 的建議清單上傳 Knowledge Files。
>
> 📌 **OpenAI Codex CLI 使用者**：請讀取 [`AGENTS.md`](./AGENTS.md) 作為入口，詳細安裝步驟見 [`CODEX_SETUP.md`](./CODEX_SETUP.md)。
>
> 📌 **Google Gemini CLI 使用者**：請讀取 [`GEMINI.md`](./GEMINI.md) 作為入口，詳細安裝步驟見 [`GEMINI_SETUP.md`](./GEMINI_SETUP.md)。
>
> 📌 **OpenClaw 使用者**：將本 repo clone 至 `~/.openclaw/skills/ecpay`，詳細步驟見 [`OPENCLAW_SETUP.md`](./OPENCLAW_SETUP.md)。
...
| 嵌入付款到你的頁面（SPA/App） | **AES-JSON** | ★★★ | [guides/02](./guides/02-payment-ecpg.md) — **注意雙 Domain：Token/建立交易（GetToken、CreatePayment）走 `ecpg`；查詢/請退款走 `ecpayment`，混用會 404** |
...
│   │   ├── 📋 **串接前先確認**（讀 guides/02 §首次串接快速路徑）：後端 ReturnURL/OrderResultURL 端點均可接收 POST、前端已備妥引入外部 JS 的頁面、AES 加密函式已備妥（非 PHP）
...
│   │   ├── ⚠️ ATM/CVS/Barcode：CreatePayment 後 Data 含付款指示（虛擬帳號/超商代碼）需顯示給消費者，ReturnURL **非同步**在消費者繳費後才到（見 guides/02 §非信用卡付款）
...
├── Apple Pay → 優先站內付 2.0（完整 iOS SDK 支援，讀 guides/02 §Apple Pay）；AIO 亦可（ChoosePayment=ApplePay，讀 guides/01）[預計 30m-1h]
...
- **CheckMacValue 驗證不可使用 `==` 或 `===` 比對**（須改用 timing-safe 函式，防止 timing attack）：PHP `hash_equals()`、Python `hmac.compare_digest()`、Node.js `crypto.timingSafeEqual()`、Go `subtle.ConstantTimeCompare()`、C# `CryptographicOperations.FixedTimeEquals()`；完整語言對照表見 [guides/13](./guides/13-checkmacvalue.md)
...
- **Callback 必須實作冪等（Idempotency）與重放保護**：綠界 Callback 可能因網路異常重送最多 4 次。處理邏輯應以 `MerchantTradeNo` 為 key 做 upsert（非 insert），避免重複入帳或重複出貨。建議同時檢查 `PaymentDate` 與系統時間差異，過大時記錄警告
...
> **必須 web_fetch 的情況**：(1) 正式上線前最終確認、(2) API 回傳不符預期、(3) 需確認最新業務規則（如金額範圍）、(4) 生成 API 呼叫程式碼時。
> **無需 web_fetch 的情況**：學習串接流程、概念說明（不生成 API 呼叫程式碼）、已知參數未變動且僅用於解說時。
