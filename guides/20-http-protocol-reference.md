...
| 直播收款 | AES-JSON（請求）/ AES-JSON+CMV（callback） | application/json | AES Data（callback 含 CheckMacValue，ECTicket 式 SHA256） | AES-128-CBC + SHA256 | ecpayment.ecpay.com.tw | [18](./18-livestream-payment.md) |
...
| 服務 | 正式環境 | 測試環境 |
|------|---------|---------|
| 站內付 2.0 Token（GetTokenbyTrade/GetTokenbyUser） | ecpg.ecpay.com.tw | ecpg-stage.ecpay.com.tw |
...
| 電子票證 | ecticket.ecpay.com.tw | ecticket-stage.ecpay.com.tw | ※AES-JSON+CMV，見 §2.4 |

> **注意**：ECPG 使用兩個不同 domain — Token 類及建立交易（GetToken、GetTokenByTrade、CreatePayment）走 `ecpg`，查詢/請退款（QueryTrade、DoAction）走 `ecpayment`。
...
> | **電子票證** | `Timestamp` + `Revision: "1.0.0"` | `{"Timestamp": 1709618400, "Revision": "1.0.0"}` |
>
...
{
  "MerchantID": "3085676",
  "RqHeader": { "Timestamp": 1709618400, "Revision": "1.0.0" },
  "Data": "Base64EncodedAESEncryptedString...",
...
4. **重試機制依服務不同**：
   - AIO 金流 / 電子票證：每 5-15 分鐘重送，每日最多 4 次
   - 站內付 2.0 / 幕後授權 / 物流：約每 2 小時重試
...
- [references/Payment/站內付2.0API技術文件Web.md](../references/Payment/站內付2.0API技術文件Web.md) — 站內付 2.0 API 規格（34 URLs）
