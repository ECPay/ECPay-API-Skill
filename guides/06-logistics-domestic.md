...
- B2C 測試帳號：MerchantID `2000132` / HashKey `5294y06JbISpM5x9` / HashIV `v77hoKGq4kWxNNIS`
...
- [ ] 先測超商取貨（CVS），比宅配流程簡單
- [ ] 電子地圖選店的 ServerReplyURL 與物流狀態的 ServerReplyURL 是**兩個不同的 URL**
...
| `MerchantTradeNo` | String(20) | ✅ 必填 | 每次唯一；英數字 |
| `MerchantTradeDate` | String(20) | ✅ 必填 | 格式：`'yyyy/MM/dd HH:mm:ss'` |
...
| `GoodsName` | String(50) | ✅ 必填 | 商品名稱 |
| `SenderName` | String(10) | ✅ 必填 | 寄件人姓名 |
| `SenderCellPhone` | String(20) | ✅ 必填 | 寄件人手機 |
...
$input = [
    ...
    'IsCollection'     => 'N',
    'ServerReplyURL'   => 'https://你的網站/ecpay/map-result',  // 收取選店結果
];
...
$storeID   = $_POST['CVSStoreID']   ?? null;  // 門市代碼（建立物流訂單時使用）
...
$input = [
    ...
    'ServerReplyURL'    => 'https://你的網站/ecpay/logistics-notify',  // 物流狀態通知
];
