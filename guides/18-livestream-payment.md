...
| 項目 | 規格 |
|------|------|
| 協議模式（API 請求） | AES-JSON — 詳見 [guides/20-http-protocol-reference.md](./20-http-protocol-reference.md) |
| HTTP 方法 | POST |
| Content-Type | `application/json` |
| 認證 | AES-128-CBC 加密 Data 欄位 — 詳見 [guides/14-aes-encryption.md](./14-aes-encryption.md) |
| 回應結構 | 三層 JSON（TransCode → 解密 Data → RtnCode） |
| ReturnURL Callback | JSON POST (`application/json`)；驗證：① `TransCode === 1` ② CheckMacValue（timing-safe，ECTicket 公式 `strtoupper(SHA256(toLowerCase(urlencode(HashKey+Data明文+HashIV))))`，**無 .NET 字元替換、無參數排序**）③ AES 解密 Data ④ `RtnCode === 1`（**整數**）；**回應純文字 `1\|OK`** |
...
> Base domain 使用 ECPG **交易 domain**（`ecpayment`，不是 Token domain `ecpg`）：測試 `ecpayment-stage.ecpay.com.tw`，正式 `ecpayment.ecpay.com.tw`。
...
- **付款通知**：消費者付款後，綠界以 **JSON POST** 傳送到 ReturnURL（格式同 ECTicket：AES 解密 Data + ECTicket-式 CMV，**非** AIO Form POST 格式）；驗證順序：TransCode === 1 → CMV timing-safe 驗證 → AES 解密 → RtnCode === 1（**整數**）；**回應純文字 `1|OK`**（⚠️ 與電子票證不同 — ECTicket 回 AES+CMV，直播收款回純文字）
