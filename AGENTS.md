...
> **V1.0** | 適用 OpenAI Codex CLI | 完整知識庫入口：`SKILL.md`
...
若尚未 clone，請參閱 [`CODEX_SETUP.md`](./CODEX_SETUP.md) 完成安裝。
...
- 退款 → 當天：`Action=N`（AIO）/ 隔日後：`Action=R`；非信用卡無 API 退款
...
5. **AES-JSON 需雙層錯誤檢查**：先 `TransCode`，再 `RtnCode`。電子票證需三層（TransCode → CheckMacValue → RtnCode）。
6. **ECPG 使用兩個 domain**：Token API 用 `ecpg.ecpay.com.tw`，交易 API 用 `ecpayment.ecpay.com.tw`，混用會 404。
...
|------|-----------|---------|--------|------|
| AIO / ECPG / 幕後授權 | 3002607 | pwFHCqoQZGmho4w6 | EkRm7iFT261dpevs | SHA256 / AES |
...
| 全方位物流 | 2000132 | 5294y06JbISpM5x9 | v77hoKGq4kWxNNIS | AES |
...
**生成 API 呼叫程式碼時**，必須先使用 `web_fetch` 讀取 `references/` 中對應 URL，取得官方最新規格（確保不使用過時參數）。正式上線前亦須重新確認。
